...
| `wait_for_input` | `variable_name` (default `"user_input"`), `input_kind` ∈ `text` `contact` `location` `photo` `document`, `button_text`, `input_placeholder` (tokens ok, ≤64 chars), `timeout_value`, `timeout_unit` ∈ `seconds` `minutes` `hours`, `media_url`, `reactive` | Sends the prompt, halts until the visitor answers with the right kind; the answer lands in `user.<variable_name>`. `contact` → `{phone_number, first_name, last_name, user_id}`; `location` → `{latitude, longitude}` from the button, `{address}` when typed instead; `photo` → `{file_id}`; `document` → `{file_id, file_name, mime_type}`. `contact`/`location` show a one-time [request button](https://core.telegram.org/bots/api#keyboardbutton) labelled `button_text`. `input_placeholder` is the grey hint in the visitor's input field: a text wait sends it on a [ForceReply](https://core.telegram.org/bots/api#forcereply) (the answer arrives as a reply to the prompt), a request kind adds it to its button's keyboard. Slots `answer` (0) and `timeout` (1) — the timer arms only when a `timeout` child exists. |
